...
# which Omarchy's stock hyprland.lua already requires (Lua files only,
# as of Omarchy 4 "Quattro"), so no stowed config file is touched.
# System-level tweaks are applied via sudo with grep-guards and
# timestamped backups.
#
...
write_hypr_extras() {
    ...
    # Omarchy 4 (Quattro) switched Hyprland config to Lua. Its toggles loader
    # requires *.lua only, so *.conf files here are silently ignored. Drop any
    # left over from a pre-Quattro run of this script.
    rm -f "$TOGGLES_DIR/10-bootstrap-chrome.conf" "$TOGGLES_DIR/20-bootstrap-laptop.conf"

    local chrome_lua="$TOGGLES_DIR/10-bootstrap-chrome.lua"
...
EOF
        info "Wrote $chrome_lua"
    elif [[ -f "$chrome_lua" ]]; then
        rm -f "$chrome_lua"
        info "Removed $chrome_lua (autostart disabled)"
    fi

    # Omarchy's default/hypr/nvidia.lua gates these behind o.shell_succeeds(),
    # which does not fire during Hyprland's config parse, so NVIDIA VA-API
    # decode silently breaks. Set them explicitly; values match what Omarchy
    # would set on a GSP-capable card.
    local nvidia_lua="$TOGGLES_DIR/30-nvidia-env.lua"
    if command -v omarchy-hw-nvidia >/dev/null 2>&1 && omarchy-hw-nvidia >/dev/null 2>&1; then
        cat > "$nvidia_lua" <<'EOF'
-- Generated by ~/.dotfiles/setup.sh — NVIDIA environment.
-- Explicit because Omarchy's default/hypr/nvidia.lua gates these behind
-- o.shell_succeeds(), which does not fire during Hyprland's config parse.
-- Harmless if Omarchy's detection starts working: identical values.
hl.env("NVD_BACKEND", "direct")
hl.env("LIBVA_DRIVER_NAME", "nvidia")
hl.env("__GLX_VENDOR_LIBRARY_NAME", "nvidia")
EOF
        info "Wrote $nvidia_lua"
    elif [[ -f "$nvidia_lua" ]]; then
        rm -f "$nvidia_lua"
        info "Removed $nvidia_lua (no NVIDIA GPU)"
    fi

    local laptop_lua="$TOGGLES_DIR/20-bootstrap-laptop.lua"
...
-- NOTE: Omarchy 4 ships its own lid handling (omarchy-system-lid-close /
-- omarchy-hyprland-monitor-clamshell). These unbinds replace it; drop this
-- block if you'd rather use Omarchy's clamshell behaviour.
...
o.bind("switch:off:Lid Switch", nil, 'hyprctl keyword monitor "eDP-1, preferred, auto, 1"', { locked = true })

-- Power button => shutdown (instead of the Omarchy power menu).
hl.unbind("XF86PowerOff")
o.bind("XF86PowerOff", "Shutdown", "omarchy-system-shutdown", { locked = true })
EOF
        info "Wrote $laptop_lua"
    elif [[ -f "$laptop_lua" ]]; then
        rm -f "$laptop_lua"
        info "Removed $laptop_lua (desktop chassis)"
    fi
}
